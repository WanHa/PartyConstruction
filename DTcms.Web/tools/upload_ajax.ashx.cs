...
namespace DTcms.Web.tools
{
    ...
    public class upload_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void EditorFile(HttpContext context)
        {
            ...
            Qiniu.Common.Config.AutoZone(info.P_AK, info.P_Scope, false);
            Mac mac = new Mac(info.P_AK, info.P_SK);
            string bucket = info.P_Scope + ":" + imgFile.FileName;
            string saveKey = imgFile.FileName;
            // 上传策略，参见 
            ...
            try
            {
                ...
                var result = fu.UploadStream(imgFile.InputStream, saveKey, token);
            }
            catch (Exception)
            {
                showError(context, "上传过程中发生意外错误！");
                return;
            }
            #endregion
            ...
            hash["error"] = 0;
            hash["url"] = info.P_RootUrl + imgFile.FileName;
