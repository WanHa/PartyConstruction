        ...
        private void get_navigation_childs(HttpContext context, DataTable oldData, int parent_id, int role_type, List<Model.manager_role_value> ls)
        {
            ...
            for (int i = 0; i < dr.Length; i++)
            {
                ...
                if (parent_id == 0)
                {
                    ...
                    // 2017-07-27 李国勋添加，因党建需求点击首页需要注销后返回地图页面.
                    if ("首页".Equals(title))
                    {
                        context.Response.Write("<h2><div id='userLogout' style='width:180px;'onclick='UserLogout()'><i style=\"background-image: url('" + dr[i]["icon_url"].ToString()
                        + "')\"></i><span  class='color:#000;'>" + dr[i]["title"].ToString() + "</span></div></h2>\n");
                    }
                    else
                    {
                        context.Response.Write("<h2><i style=\"background-image: url('" + dr[i]["icon_url"].ToString()
                            + "')\"></i><span>" + dr[i]["title"].ToString() + "</span></h2>\n");
                    }
                    //调用自身迭代
                    ...
                    if (orderConfig.confirmmsg > 0)
                    {
                        switch (orderConfig.confirmmsg)
                        {
                            ...
                            case 2: //邮件通知
                                ...
                                //发送邮件
                                DTMail.sendMail(siteConfig.emailsmtp, siteConfig.emailssl, siteConfig.emailusername,
                                    DESEncrypt.Decrypt(siteConfig.emailpassword, siteConfig.sysencryptstring),
                                    siteConfig.emailnickname, siteConfig.emailfrom, model.email, mailTitle, mailContent);
                                break;
                        }
                    }
                    #endregion
