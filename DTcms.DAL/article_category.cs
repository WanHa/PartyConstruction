...
namespace DTcms.DAL
{
    ...
    public partial class article_category
    {
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append("channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,content,seo_title,seo_keywords,seo_description)");
                        ...
                        string imageurl = name.GetQiNiuFileUrl(model.img_url);
                        StringBuilder str = new StringBuilder();
                        str.Append("insert into P_Image(");
                        str.Append("P_Id,P_ImageId,P_ImageUrl,P_CreateTime,P_Status,P_PictureName,P_ImageType) ");
                        str.Append("values(");
                        str.Append("@P_Id,@P_ImageId,@P_ImageUrl,@P_CreateTime,@P_Status,@P_PictureName,@P_ImageType) ");
                        ...
                        parameters2[5].Value = model.img_url;
                        parameters2[6].Value = (int) ImageTypeEnum.栏目;
                        object urlobj = DbHelperSQL.GetSingle(conn, trans, str.ToString(), parameters2); //带事务
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        string imageurl = name.GetQiNiuFileUrl(model.img_url);
                        StringBuilder str = new StringBuilder();
                        str.Append("update P_Image set P_ImageUrl = @P_ImageUrl,P_PictureName = @P_PictureName where P_ImageId = '"+model.id+ "' and P_Status = 0 and P_ImageType = 20012");
                        SqlParameter[] parameters2 = {
                            new SqlParameter("@P_ImageUrl",SqlDbType.NVarChar,200),
                            new SqlParameter("@P_PictureName",SqlDbType.NVarChar,100)
                        };
                        ...
                        DbHelperSQL.ExecuteSql(conn, trans, str.ToString(), parameters2);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,content,seo_title,seo_keywords,seo_description ");
            strSql.Append(",(select P_PictureName from P_Image LEFT JOIN  " + databaseprefix + "article_category ON " + databaseprefix + "article_category.id = P_Image.P_ImageId  where P_Image.P_ImageType = 20012 and P_Image.P_Status = 0 and P_Image.P_ImageId = '"+id+"')  as img_url");
